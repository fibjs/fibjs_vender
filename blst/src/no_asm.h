...
{
    llimb_t limbx;
    limb_t mask, borrow, mx, hi, carry;
    limb_t *tmp = alloca(sizeof(limb_t)*(n+1));
    size_t i, j;
                      ...
                      const limb_t p[], size_t n)
{
    ...
    limb_t *tmp = alloca(sizeof(limb_t)*n);
    size_t i;
                           ...
                           size_t n)
{
    ...
    limb_t mask, carry, borrow;
    limb_t *tmp = alloca(sizeof(limb_t)*n);
    limb_t *two_a = alloca(sizeof(limb_t)*n);
    size_t i;
                       ...
                       const limb_t p[], size_t n)
{
    ...
    flag &= vec_is_zero(a, sizeof(limb_t)*n) ^ 1;
    mask = (limb_t)0 - flag;
...
{
    limb_t zero;
    limb_t *ret_ = alloca(sizeof(limb_t)*n);
    limb_t *a_ = alloca(sizeof(limb_t)*n);
    limb_t *b_ = alloca(sizeof(limb_t)*n);

    limbs_from_le_bytes(a_, a, sizeof(limb_t)*n);
    limbs_from_le_bytes(b_, b, sizeof(limb_t)*n);

    add_mod_n(ret_, a_, b_, p, n);
    zero = vec_is_zero(ret_, sizeof(limb_t)*n);

    le_bytes_from_limbs(ret, ret_, sizeof(limb_t)*n);
...
{
    llimb_t limbx;
    limb_t mask, carry, borrow, mx, hi;
    limb_t *tmp = alloca(sizeof(limb_t)*n);
    const limb_t *b = a;
    size_t i, j;
                              ...
                              const limb_t p[], limb_t n0, size_t n)
{
    ...
    vec_copy(ret, tmp, sizeof(limb_t)*n);
}
...
static limb_t smul_n_shift_n(limb_t ret[], const limb_t a[], limb_t *f_,
                                           ...
                                           size_t n)
{
    limb_t f, g, neg, carry, hi;
    limb_t *a_ = alloca(sizeof(limb_t)*(n+1));
    limb_t *b_ = alloca(sizeof(limb_t)*(n+1));
    size_t i;
...
{
    limb_t neg, hi;
    limb_t *u_ = alloca(sizeof(limb_t)*n);
    limb_t *v_ = alloca(sizeof(limb_t)*n);
...
{
    llimb_t limbx;
    limb_t *a = alloca(sizeof(limb_t)*n);
    limb_t *b = alloca(sizeof(limb_t)*n);
    limb_t *u = alloca(sizeof(limb_t)*n*2);
    limb_t *v = alloca(sizeof(limb_t)*n*2);
    limb_t *t = alloca(sizeof(limb_t)*n*2);
    limb_t a_[2], b_[2], sign, carry, top;
    factors fg;
    size_t i;

    vec_copy(a, inp, sizeof(limb_t)*n);
    vec_copy(b, mod, sizeof(limb_t)*n);
    vec_zero(u, sizeof(limb_t)*n*2); u[0] = 1;
    vec_zero(v, sizeof(limb_t)*n*2);
    ...
    for (i=0; i<(2*n*LIMB_T_BITS)/(LIMB_T_BITS-2); i++) {
        ...
        vec_copy(a, t, sizeof(limb_t)*n);
        smul_2n(t, u, fg.f0, v, fg.g0, 2*n);
        smul_2n(v, u, fg.f1, v, fg.g1, 2*n);
        vec_copy(u, t, sizeof(limb_t)*n*2);
    }
...
static bool_t ct_is_sqr_mod_n(const limb_t inp[], const limb_t mod[], size_t n)
{
    limb_t *a = alloca(sizeof(limb_t)*n);
    limb_t *b = alloca(sizeof(limb_t)*n);
    limb_t *t = alloca(sizeof(limb_t)*n);
