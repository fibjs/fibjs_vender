function(queryhostarch RETVAL)
    string(TOLOWER ${CMAKE_HOST_SYSTEM_PROCESSOR} RETARCH)
    
    if(${RETARCH} MATCHES "^(i386)|(i686)|(x86)$")
        set(${RETVAL} i386 PARENT_SCOPE)
    elseif(${RETARCH} MATCHES "^(x86_64)|(amd64)|(AMD64)$")
        set(${RETVAL} amd64 PARENT_SCOPE)
    elseif(${RETARCH} MATCHES "^(armv6)|(armv7)|(armv7s)|(armv7l)$")
        set(${RETVAL} arm PARENT_SCOPE)
    elseif(${RETARCH} MATCHES "^aarch64$")
        set(${RETVAL} arm64 PARENT_SCOPE)
    elseif(${RETARCH} MATCHES "^(mips)|(mipsel)$")
        set(${RETVAL} mips PARENT_SCOPE)
    elseif(${RETARCH} MATCHES "^mips64$")
        set(${RETVAL} mips64 PARENT_SCOPE)
    elseif(${RETARCH} MATCHES "^powerpc$")
        set(${RETVAL} ppc PARENT_SCOPE)
    elseif(${RETARCH} MATCHES "^ppc64$")
        set(${RETVAL} ppc64 PARENT_SCOPE)
    else()
        set(${RETVAL} ${RETARCH} PARENT_SCOPE)
    endif()
endfunction()

queryhostarch(HOST_ARCH)
