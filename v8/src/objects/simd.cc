...
void AtomicUint8ArrayToHexSlow(const char* bytes, size_t length,
                               DirectHandle<SeqOneByteString> string_output) {
  int index = 0;
  // Use volatile to ensure atomic-like read behavior for shared memory.
  // std::atomic_ref is C++20, so we use volatile as a fallback.
  const volatile char* volatile_bytes =
      reinterpret_cast<const volatile char*>(bytes);
  for (size_t i = 0; i < length; i++) {
    uint8_t byte = static_cast<uint8_t>(volatile_bytes[i]);
    PerformNibbleToHexAndWriteIntoStringOutPut(byte, index, string_output);
    index += 2;
  }
}
...
bool ArrayBufferFromHex(const base::Vector<T>& input_vector, bool is_shared,
  ...
  for (uint32_t i = 0; i < output_length * 2; i += 2) {
    ...
    if (result.has_value()) {
      if (is_shared) {
        // Use volatile to ensure atomic-like write behavior for shared memory.
        // std::atomic_ref is C++20, so we use volatile as a fallback.
        volatile uint8_t* volatile_buffer =
            reinterpret_cast<volatile uint8_t*>(buffer);
        volatile_buffer[index++] = result.value();
      } else {
