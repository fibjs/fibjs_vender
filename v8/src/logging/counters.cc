...
static exlib::fiber_local<base::LazyInstance<
      std::unordered_map<const TimedHistogram*, bool>>::type> active_timer(LAZY_INSTANCE_INITIALIZER);
  bool is_running = (*active_timer.get().Pointer())[this];
  DCHECK_NE(is_running, expect_to_run);
  (*active_timer.get().Pointer())[this] = !is_running;
  return true;
}
#endif
