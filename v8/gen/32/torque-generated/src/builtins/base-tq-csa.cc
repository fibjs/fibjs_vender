...
TF_BUILTIN(FastCreateDataProperty, CodeStubAssembler) {
  ...
  if (block24.is_used()) {
    ...
    CodeStubAssembler(state_).BuildAppendJSArray(ElementsKind::PACKED_DOUBLE_ELEMENTS, TNode<JSArray>{tmp0}, TNode<JSAny>{parameter3}, &label19);
    ca_.Goto(&block27);
    if (label19.is_used()) {
      ca_.Bind(&label19);
      ca_.Goto(&block28);
  ...
  if (block29.is_used()) {
    ...
    CodeStubAssembler(state_).BuildAppendJSArray(ElementsKind::HOLEY_DOUBLE_ELEMENTS, TNode<JSArray>{tmp0}, TNode<JSAny>{parameter3}, &label22);
    ca_.Goto(&block32);
    if (label22.is_used()) {
      ca_.Bind(&label22);
      ca_.Goto(&block33);
  ...
  if (block30.is_used()) {
    ...
    CodeStubAssembler(state_).BuildAppendJSArray(ElementsKind::HOLEY_ELEMENTS, TNode<JSArray>{tmp0}, TNode<JSAny>{parameter3}, &label23);
    ca_.Goto(&block38);
    if (label23.is_used()) {
      ca_.Bind(&label23);
      ca_.Goto(&block39);
...
void VerifiedUnreachable_0(compiler::CodeAssemblerState* state_) {
  ...
  if (block0.is_used()) {
    ca_.Bind(&block0);
    tmp0 = FromConstexpr_bool_constexpr_bool_0(state_, false);
    CodeStubAssembler(state_).StaticAssert(TNode<BoolT>{tmp0}, "static_assert(false) at https://source.chromium.org/chromium/chromium/src/+/main:v8/src/builtins/base.tq?l=2134&c=3");
    CodeStubAssembler(state_).Unreachable();
  }
}
