...
// glibc provides execinfo.h with backtrace support
#include <execinfo.h>
#include <cxxabi.h>
#define HAVE_BACKTRACE 1
#else
// musl libc (Alpine Linux) and other libc implementations
// may not have execinfo.h, so we disable backtrace support
#include <cxxabi.h>
...
extern "C" void ex_print_stack_trace()
{
    ...
    SymCleanup(process);
#elif HAVE_BACKTRACE
    // POSIX implementation using execinfo (glibc)
    void* stack[64];
...
#else
    // No backtrace support (e.g., musl libc on Alpine Linux)
